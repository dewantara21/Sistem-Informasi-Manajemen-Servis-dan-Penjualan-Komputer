...
if(isset($_POST['btnSimpan'])){
	...
	$cekSql="SELECT * FROM level WHERE level='$txtNama' AND NOT(level='".$_POST['txtLama']."')";
	...
	if (count($pesanError)>=1 ){
		...
		$mySql	= "UPDATE level SET level='$txtNama' WHERE kd_level ='".$_POST['txtKode']."'";
...
$mySql	 = "SELECT * FROM level WHERE kd_level='$Kode'";
$myQry	 = mysql_query($mySql, $koneksidb)  or die ("Query ambil data salah : ".mysql_error());
$myData	 = mysql_fetch_array($myQry);

	// Menyimpan data ke variabel temporary (sementara)
	$dataKode	= $myData['kd_level'];
	$dataNama	= isset($_POST['txtNama']) ? $_POST['txtNama'] : $myData['level'];
?>
...
<table class="table-list"  cellpadding="2" cellspacing="1" width="100%">
	<div class="row-fluid">
	<div class="span12">
	  <h3 class="header smaller lighter blue">UBAH  LEVEL </h3></div></div>
	...
	<tr>
	  ...
	  <td><b>:</b></td>
	  <td><input id="nama" name="txtNama" title="Nama Level Tidak Boleh Kosong" type="text" value="<?php echo $dataNama; ?>" size="70" maxlength="100" class="required" onkeypress="return huruf(event)" />
      <input name="txtLama" type="hidden" value="<?php echo $myData['level']; ?>" /></td></tr>
	<tr><td>&nbsp;</td>
	  <td>&nbsp;</td>
